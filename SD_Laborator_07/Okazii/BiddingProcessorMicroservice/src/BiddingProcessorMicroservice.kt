    ...
    init {
        ...
        receiveProcessedBidsObservable = Observable.create<String> { emitter ->
            while (true) {
                ...
                // daca se primeste un mesaj gol (NULL), atunci inseamna ca cealalta parte a socket-ului a
                // fost inchisa
                ...
                // daca mesajul este cel de tip „FINAL DE LISTA DE MESAJE” (avand corpul "final"),
                // atunci se emite semnalul Complete
                if (Message.deserialize(receivedMessage.toByteArray()).body == "final") {
                    emitter.onComplete()

                    // s-au primit toate mesajele de la MessageProcessorMicroservice, i se trimite un mesaj
                    // pentru a semnala
                    // acest lucru
