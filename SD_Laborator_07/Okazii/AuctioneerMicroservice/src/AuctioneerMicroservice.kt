...
class AuctioneerMicroservice {
    ...
    private val bidderConnections: MutableMap<Socket, String> =
        mutableMapOf() // map in loc de list pt ca mike o vrut nume, phone si adresa

    companion object Constants {
        const val MESSAGE_PROCESSOR_HOST = "localhost"
        const val MESSAGE_PROCESSOR_PORT = 2100
        const val AUCTIONEER_PORT = 2000
    ...
    init {
        ...
        // se creeaza obiectul Observable cu care se genereaza evenimente cand se primesc
        // oferte de la bidderi
            ...
            while (true) {
                try {
                    ...
                    val bufferReader = BufferedReader(InputStreamReader(
                        bidderConnection.inputStream))
                    val receivedMessage = bufferReader.readLine()

                    // daca se primeste un mesaj gol (NULL), atunci inseamna ca cealalta
                    // parte a socket-ului a fost inchisa
                    if (receivedMessage == null) {
                        // deci subscriber-ul respectiv a fost deconectat
                        bufferReader.close()
                        bidderConnection.close()

                        emitter.onError(Exception("Eroare: Bidder-ul ${bidderConnection.port} " +
                                "a fost deconectat."))
                    }

                    val message = Message.deserialize(receivedMessage.toByteArray())
                    bidderConnections[bidderConnection] = message.sender

                    // se emite ce s-a citit ca si element in fluxul de mesaje
                    emitter.onNext(receivedMessage)
                } catch (e: SocketTimeoutException) {
                    // daca au trecut cele 15 secunde de la pornirea licitatiei, inseamna ca
    ...
    private fun finishAuction() {
        val MAX_RETRIES      = 10
        val RETRY_TIMEOUT_MS = 6_000  // are 6 secunde per incercare sa se conecteze 6*10=60s

        var receivedMessage: String? = null
        var attempt = 0

        auctioneerSocket.setSoTimeout(RETRY_TIMEOUT_MS) // timeout pt biddingprocessor

        while (receivedMessage == null && attempt < MAX_RETRIES) {
            attempt++
            // se asteapta rezultatul licitatiei
            try {
                val biddingProcessorConnection = auctioneerSocket.accept()
                val bufferReader = BufferedReader(InputStreamReader(
                    biddingProcessorConnection.inputStream))
        ...
        if (receivedMessage == null) {
            println("BiddingProcessor nu a raspuns dupa $MAX_RETRIES tentative. " +
                    "Licitatia nu poate fi finalizata.")
            auctioneerSocket.close()
            exitProcess(1)
        }
        ...
        println("Am primit rezultatul licitatiei de la BiddingProcessor: " +
                "${result.sender} a castigat cu pretul: $winningPrice")
        ...
        bidderConnections.forEach { (socket, identity) ->
            if (identity == result.sender) {
                socket.getOutputStream().write(winningMessage.serialize())
            } else {
                socket.getOutputStream().write(losingMessage.serialize())
            }
            socket.close()
        }
