...
class ClientMicroservice {
    companion object {
        ...
        // adica sa o generez eu dinainte
        // BD-ul de la pagina 17
    }

    @Bean
    @InboundChannelAdapter(value = Source.OUTPUT, poller = [Poller(fixedDelay =
        "10000", maxMessagesPerPoll = "1")])
    fun comandaProdus(): () -> Message<String> {
        return {
            val produsComandat = listaProduse[(0 until listaProduse.size).
                                                shuffled()[0]] // [0] => 1 sg produs
            val cantitate: Int = Random.nextInt(1, 100)
            val identitateClient = "Popescu Ion"
            val adresaLivrare = "Codrii Vlasiei nr 14"

            ///TODO - datele clientului sa fie citite dinamic si inregistrate in
            // baza de date
            // "dinamic" - a se citi de la tastatura / GUI
