...
class MessageProcessor:
    def __init__(self, bids_topic, notify_message_processor_topic,
        ...
        # consumatorul notificarii de la Auctioneer cum ca s-a terminat
        # licitatia
    ...
    def get_and_process_messages(self):
        ...
        # a ajuns prima notificare, se asteapta si celelalte notificari
        # timp de maxim 15 secunde
        self.notify_message_processor_consumer.config[
    ...
    def finish_processing(self, sorted_bids):
        ...
        for bid in sorted_bids:
            for header in bid.headers: # bid.headers e o lista de tuple, de forma
                # [
                # ("amount", b'1234'),
                # ("identity", b'Bidder 1234-5678-90ab')
                # ]
                # header[0] = amount sau identity
                if header[0] == "identity":
                    identity = str(header[1], encoding="utf-8")
                elif header[0] == "amount":
                    bid_amount = int.from_bytes(header[1], 'big')
            print("[{}] {} a licitat {}.".format(
                datetime.fromtimestamp(bid.timestamp / 1000),
                identity, bid_amount))

            # se stocheaza mesajele ordonate dupa timestamp si fara duplicate intr-un topic
            # separat
