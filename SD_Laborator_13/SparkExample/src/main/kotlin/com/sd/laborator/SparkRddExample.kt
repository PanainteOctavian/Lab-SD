...
fun main(args: Array<String>) {
    ...
    val items = listOf("123/643/7563/2134/ALPHA", "2343/6356/BETA/2342/12",
        "23423/656/343")
    // paralelizarea colectiilor
    val distributedDataset = sparkContext.parallelize(items)

    // 1) spargerea fiecarui string din lista intr-o lista de substring-uri
    //      si reunirea intr-o singura lista
    ...
    // setul de date nu este inca incarcat in memorie (si nu se actioneaza
    // inca asupra lui)
    val lines = sparkContext.textFile("src/main/resources/data.txt")

    // pentru utilizarea unui RDD de mai multe ori, trebuie apelata metoda
    // persist:
    lines.persist(StorageLevel.MEMORY_ONLY())

    // functia de mapare reprezinta o transformare a setului de date initial
    // (nu este calculat imediat)
    // abia cand se ajunge la functia de reducere (care este o actiune) Spark
    // imparte operatiile in task-uri
    // pentru a fi rulate pe masini separate (fiecare masina executand o
    // parte din map si reduce)
    // exemplu cu functii lambda:
    val totalLength0 = lines.map { s->s.length }.reduce { a: Int,
                                                          b: Int -> a + b }
    ...
    // maniera eficienta:
    val broadcastVar: Broadcast<List<Int>> = sparkContext.broadcast(
        listOf(1, 2, 3))
    val totalLength3 = lines.map { s->s.length + broadcastVar.value()[0] }.
