...
fun main(args: Array<String>) {
    ...
    val df: Dataset<Row> = sparkSession.sqlContext().read().json(
    ...
    val sqlDF: Dataset<Row> = sparkSession.sql(
        "SELECT * FROM people")
    sqlDF.show()
    ...
    // Aplicarea unei scheme pe un RDD de Java Bean-uri pentru a obtine
    // DataFrame
    val peopleDF: Dataset<Row> = sparkSession.createDataFrame(peopleRDD,
        Person::class.java)
    // Inregistrarea DataFrame-ului ca un view temporar
    peopleDF.createOrReplaceTempView("people")

    // Selectarea persoanelor intre 13 si 19 ani cu o interogare SQL
    val teenagersDF: Dataset<Row> = sparkSession.sql(
        "SELECT name FROM people WHERE age BETWEEN 13 AND 19")

    // Coloanele dintr-un Row din rezultat pot fi accesate dupa indexul
    // coloanei
    val stringEncoder = Encoders.STRING()
    val teenagerNamesByIndexDF = teenagersDF.map(
        MapFunction { row: Row -> "Name: " +
                row.getString(0) } as MapFunction<Row, String>,
        stringEncoder
    )
